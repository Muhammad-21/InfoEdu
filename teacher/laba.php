<?php
    ...
    if($_SESSION['file_status']==0){
        ...
        $status='Неправильный номер занятии';
...
<div style="border: 3px solid Navy;margin:5%;">
    ...
    <div style="border: 1px solid #dfe4e9;padding:1%;" class="container"><br>
        <form action="laba_load.php" method="POST" enctype="multipart/form-data">
        <?php if($color==1){
        ?><div style="color: green;"><?php echo $status;$_SESSION['file_status']=3;?></div><?php
            ...
            <input style="margin-left:1%;" type="number" name="number" placeholder="Номер занятии"  min=1 required>
            ...
            <!-- <option>Материал для курсовой работы</option> -->
            </select><br>
...
<script>
            ...
            for(let i=0;i<=deleteEl.length-1;i++){
                deleteEl[i].onclick =()=> {
                   delEl[i].remove();
                   location.href = "delete_laba.php?link=" + linkEl[i].href;
                }
            }
</script>
</body>
