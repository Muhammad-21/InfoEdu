...
<div class="d-flex align-items-center flex-column flex-md-row p-3 px-md-4  shadow-sm">
    ...
    <a class="btn" style="color: white; position:absolute; margin-left:70%; bottom:90%;" href="laba.php">Вернуться назад</a>
    </nav>
</div>
</div>
<div style="border: 3px solid Navy;margin:5%;padding:2%;"><br>
    ...
    WHERE work.work_type='дз' AND work.id_teacher=3
    ...
    <script>
            ...
            for(let i=0;i<=deleteEl.length-1;i++){
                deleteEl[i].onclick =()=> {
                   location.href = "delete_dz.php?id=" + deleteEl[i].name+"&message="+mesEl[i].value+"&type="+1;
                }
                delEl[i].onclick = () =>{
                    location.href = "accept_dz.php?id="+ delEl[i].name+"&mark="+markEl[i].value+"&type="+1;
                }
            }
    </script>
</body>
